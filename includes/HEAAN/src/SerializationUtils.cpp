...
Ciphertext* SerializationUtils::readCiphertextModified(Ciphertext& cipher, string path) {
    long n, logp, logq;
    fstream fin;
    fin.open(path, ios::binary|ios::in);
    fin.read(reinterpret_cast<char*>(&n), sizeof(long));
...
Key* SerializationUtils::readKeyModified(Key& key, string path) {
    fstream fin;
    fin.open(path, ios::binary|ios::in);
    if (fin.is_open())
        cout << "Key found lol" << endl;
