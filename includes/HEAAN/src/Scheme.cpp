...
void Scheme::read_encryption_key() {
    cout << "Eussou ?" << endl;
    Key* encryption_key = new Key();
    cout << "Oula" << endl;
    string path = "serkey/ENCRYPTION_KEY.txt";
    serKeyMap.insert(pair<long, string>(ENCRYPTION, path));
    cout << "La CHANCLA" << endl;
    SerializationUtils::readKeyModified(*encryption_key, path);
    keyMap.insert(pair<long, Key*>(ENCRYPTION, encryption_key));

    cout << "Encryption key" << endl;
}


void Scheme::read_multiplication_key() {
    Key* multiplication_key = new Key();
    string path = "serkey/MULTIPLICATION_KEY.txt";
    serKeyMap.insert(pair<long, string>(MULTIPLICATION, path));

    cout << "Multiplication key" << endl;

    SerializationUtils::readKeyModified(*multiplication_key, path);
    keyMap.insert(pair<long, Key*>(MULTIPLICATION, multiplication_key));
}


void Scheme::read_left_rotation_key() {
    cout << "So, left rotations.." << endl;
    for (long i = 0; i < logN - 1; ++i) {
        cout << "Rot " << i << endl;
        Key* rotation_key = new Key();
        long idx = 1 << i;
        string path = "serkey/ROTATION_" + to_string(idx) + ".txt";
        serKeyMap.insert(pair<long, string>(idx, path));

        SerializationUtils::readKeyModified(*rotation_key, path);
        if(leftRotKeyMap.find(idx) == leftRotKeyMap.end() && serLeftRotKeyMap.find(idx) == serLeftRotKeyMap.end()) {
            leftRotKeyMap.insert(pair<long, Key*>(idx, rotation_key));
        }
    }
}


void Scheme::read_right_rotation_key() {
    cout << "So, right rotations.." << endl;
    for (long i = 0; i < logN - 1; ++i) {
        cout << "Rot " << i << endl;
        Key* rotation_key = new Key();
        long idx = Nh - (1 << i);
...
void Scheme::read_keys() {
    ...
    // MUltiplication key
    Key* multiplication_key = new Key();
    path = "serkey/MULTIPLICATION_KEY.txt";
    serKeyMap.insert(pair<long, string>(MULTIPLICATION, path));
    ...
    for (long i = 0; i < logN - 1; ++i) {
        ...
        Key* rotation_key = new Key();
        long idx = 1 << i;
        path = "serkey/ROTATION_" + to_string(idx) + ".txt";
        serKeyMap.insert(pair<long, string>(idx, path));
    ...
    /*path = "serkey/MULTIPLICATION.txt";
    serKeyMap.insert(pair<long, string>(MULTIPLICATION, path));

    Key mult_key;
    fin.open(serKeyMap.at(MULTIPLICATION), ios::binary|ios::in);
    if (fin.is_open())
        cout << "Multiplication Key found lol" << endl;
    else
        cout << "Multiplication Key not found" << endl;
    fin.read(reinterpret_cast<char*>(mult_key.rax), Nnprimes*sizeof(uint64_t));
    fin.read(reinterpret_cast<char*>(mult_key.rbx), Nnprimes*sizeof(uint64_t));
    fin.close();
...
void Scheme::encryptMsg(Ciphertext& cipher, Plaintext& plain) {
	...
	if (isSerialized)
	    cout << "LA CHANKLA" << endl;
	else
	    cout << "No serialize" << endl;
